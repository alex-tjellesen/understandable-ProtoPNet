...
While there are multiple aspects that factor into making an explanation interpretable to humans we chose to focus on a single yet important factor: to make an explanation interpretable it must consist of just a few "concepts", which for the case of ProtoPNet means that a classification should rely in just a few prototypes and not many. Thus, in order to measure this aspect of human interpretability we calculate the average number of prototypes activated for samples in the test set that the model correctly predicted. The reason why we only consider correctly predicted samples is to isolate the performance of the explanability part of the network $P$ from the overall classifier. 