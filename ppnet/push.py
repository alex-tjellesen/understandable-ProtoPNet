...
def push_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    prototype_network_parallel.module.prototype_vectors.data.copy_(torch.tensor(prototype_update, dtype=torch.float32).to("cuda" if torch.cuda.is_available() else "cpu"))
...
def update_prototypes_on_batch(search_batch_input,
    ...
    with torch.no_grad():
        search_batch = search_batch.to("cuda" if torch.cuda.is_available() else "cpu")
